...
async function loadModelsDevData(): Promise<Model<any>[]> {
	try {
		...
		// Novita exposes an OpenAI-compatible endpoint and publishes its catalog
		// under the models.dev `novita-ai` provider.
		const novitaModels = data["novita-ai"]?.models;
		for (const [modelId, model] of Object.entries(novitaModels ?? {})) {
			const m = model as ModelsDevModel;
			if (m.tool_call !== true) continue;
			models.push({
				id: modelId,
			...
			recordModelsDevReasoningOptions("novita", modelId, m);
		}
